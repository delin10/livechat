...
public class ScheduledMessageConsumer {
    ...
    @Autowired
    private List<SimpleBrokerMessageHandler> brokerMessageHandlers;
    ...
    public void schedule() {
        for (SimpleBrokerMessageHandler brokerMessageHandler : brokerMessageHandlers) {
            log.info("99000000---------{}", String.join(",", brokerMessageHandler.getDestinationPrefixes()));
        }
        try {
            ...
            for (ConsumerRecord<String, byte[]> record : records) {
                List<Message<byte[]>> messageList = MessageUtils.decode(record.value());
